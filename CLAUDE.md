...
│   │   ├── migrations/
│   │   └── test/
│   └── domain/         ← 서버 앱 공유 도메인 (entity, 비즈니스 로직)
│       └── src/
│           └── todo/
└── pnpm-workspace.yaml
```
...
`server/domain`은 npm 패키지가 아닌 TypeScript paths alias로 참조합니다.

```ts
import { Todo, TodoStatus, TodoContext } from '@domain';
```

각 서버 앱의 `tsconfig.json`에 paths 설정:
