...
```
todo-nodejs/
├── server/                    ← @todo-nodejs/server (NestJS CLI 모노레포)
│   ├── api/                   ← NestJS 앱
│   │   ├── src/
│   │   │   ├── main.ts
│   │   │   ├── app.module.ts
...
│   │   └── vitest.config.ts
│   ├── libs/
│   │   └── domain/            ← NestJS 라이브러리 (@app/domain)
│   │       └── src/
│   │           └── todo/
│   ├── envs/
│   ├── nest-cli.json
...
`server/libs/domain`은 NestJS 라이브러리로, `@app/domain` alias로 참조합니다.

```ts
import { Todo, TodoStatus, TodoContext } from '@app/domain';
```

각 앱의 `tsconfig.json`에 paths 설정:
...
# 개발 모드
pnpm dev                                        # 루트에서
pnpm --filter @todo-nodejs/server dev:api       # 직접 실행

# 빌드
pnpm --filter @todo-nodejs/server build:api     # nest build api (SWC)

# 프로덕션 실행
pnpm --filter @todo-nodejs/server start:api     # node dist/api/main.js
```
